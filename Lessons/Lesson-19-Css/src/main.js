...
class Form extends React.Component {    
    ...
    validate(validateItem) {
        <FieldValidator validateItem={validateItem}/>
    ...
    submitForm(evt) {
        const res = this.props.validators.map((validateItem) => {
            return (
                validate(validateItem)
            )
        })
        ...
        if (!res) {
            evt.preventDefault(); //отменить выполнение
                                  //нужно высветить ошибку в правильном поле  
        }
